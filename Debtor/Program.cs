...
public class Program
{
    ...
    {
        // Task 2
        var task2Debtors = debtors.Where(d => d.Email.EndsWith("@rhyta.com") || d.Email.EndsWith("@dayrep.com"));
        Debtor.PrintDebtors(task2Debtors);

        // Task 3
        var task3Debtors = debtors.Where(d => d.BirthDay >= DateTime.Now.AddYears(-36) && d.BirthDay <= DateTime.Now.AddYears(-26));
        Debtor.PrintDebtors(task3Debtors);

        // Task 4
        var task4Debtors = debtors.Where(d => d.Debt <= 5000);
        Debtor.PrintDebtors(task4Debtors);

        // Task 5
        var task5Debtors = debtors.Where(d => d.FullName.Length > 18 && d.Phone.Count(c => c == '7') >= 2);
        Debtor.PrintDebtors(task5Debtors);

        // Task 7
        var task7Debtors = debtors.Where(d => d.BirthDay.Month == 1 || d.BirthDay.Month == 2 || d.BirthDay.Month == 12);
        Debtor.PrintDebtors(task7Debtors);

        // Task 8
        var averageDebt = debtors.Average(d => d.Debt);
        var task8Debtors = debtors.Where(d => d.Debt > averageDebt).OrderBy(d => d.FullName.Split(' ')[1]).ThenByDescending(d => d.Debt);
        Debtor.PrintDebtors(task8Debtors);

        // Task 9
        var task9Debtors = debtors
    ...
    .Select(d => new
    {
        ...
        foreach (var debtor in task9Debtors)
        {
            Console.WriteLine($"{debtor.LastName}, Age: {debtor.Age}, Total Debt: {debtor.Debt}");
        }

        // Task 11
        var task11Debtors = debtors.Where(d => d.FullName.GroupBy(c => c).Any(g => g.Count() >= 3)).OrderBy(d => d.FullName);
        Debtor.PrintDebtors(task11Debtors);

        // Task 13
        var mostCommonYear = debtors.GroupBy(d => d.BirthDay.Year).OrderByDescending(group => group.Count()).First().Key;
        Console.WriteLine($"Year with the most debtors: {mostCommonYear}");

        // Task 14
        var task14Debtors = debtors.OrderByDescending(d => d.Debt).Take(5);
        Debtor.PrintDebtors(task14Debtors);
        ...
        // Task 16
        var task16Debtors = debtors.Where(d => d.BirthDay.Year < 1945);
        Debtor.PrintDebtors(task16Debtors);

        // Task 18
        var task18Debtors = debtors.Where(d => d.Phone.Distinct().Count() == d.Phone.Length).Select(d => new { d.FullName, d.Debt });
        foreach (var debtor in task18Debtors)
        {
        ...
        var task19Debtors = debtors.Where(d => d.BirthDay > today && d.Debt / ((d.BirthDay - today).Days / 30) <= 500);
        Debtor.PrintDebtors(task19Debtors);

        // Task 20
        var task20Debtors = debtors.Where(d => d.FullName.Contains("s") && d.FullName.Contains("m") && d.FullName.Contains("i") && d.FullName.Contains("l") && d.FullName.Contains("e"));
        Debtor.PrintDebtors(task20Debtors);
    }
}